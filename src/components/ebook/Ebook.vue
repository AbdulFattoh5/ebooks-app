<template>
  <div class="ebook" v-if="books">
    <div class="ebook__wrap">
      <img
      ...
      <div class="ebook__desc">
        <h1 class="ebook__title">
          {{ truncateTitle(books.volumeInfo.title) }}
        </h1>
        <p class="ebook__author ebook__info">
          Author: {{ formatAuthors(books.volumeInfo.authors) }}
        </p>
        <p class="ebook__categories ebook__info">
          Categories:
          {{ formatCategories(books.volumeInfo.categories) || " Not found" }}
        </p>
        <p class="ebook__year ebook__info">
          Published year:
          {{ getPublicationYear(books.volumeInfo.publishedDate) }}
        </p>
      </div>
    </div>
  </div>
